...
func addOrigin(key, n *Node) *Node {
	...
	// usalko:patch
	var namedMap []*Node = getNamedMap(originTag, append(getKeyLocation(key), getNamedMap("fields", getFieldLocations(n))...))
	for _, node := range namedMap { // yaegy feature for traefik 3
